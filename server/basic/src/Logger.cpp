...
static std::string getLevelString(LOG_LEVEL level){
    switch (level)
    {
...
void Logger::setLogLevel(const std::string& debugLevelStr){
    if (debugLevelStr == "FATAL")           level = LOG_LEVEL::FATAL;
        ...
        throw std::runtime_error("Log level option invalid.");
}
